<script lang="ts">
	...
	async function submit() {
		...
		try {
			...
			error = e.message || 'Gửi đánh giá thất bại';
		}
		submitting = false;
	}
</script>
...
<div>
	<a href="/location/{locationId}" class="text-sm text-primary">&larr; Quay lại</a>

	{#if submitted}
		<div class="mt-8 rounded-xl bg-card p-6 text-center">
			<p class="text-2xl">⭐</p>
			<p class="mt-2 font-semibold">Cảm ơn bạn!</p>
			<p class="mt-1 text-sm text-muted-foreground">Đánh giá của bạn đã được ghi nhận.</p>
			<a href="/location/{locationId}" class="mt-4 inline-block text-sm text-primary">Quay lại tiệm</a>
		</div>
	{:else}
		<div class="mt-4 rounded-xl bg-card p-4">
			...
			<div class="mt-4 space-y-4">
				<div>
					<p class="text-sm text-muted-foreground">Điểm đánh giá</p>
