...
<div>
	...
	<p class="mt-1 text-sm text-muted-foreground">Tong quan hoat dong hom nay.</p>

	<div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
		<div class="rounded-lg border border-border bg-card p-6 shadow-xs">
			<p class="text-sm text-muted-foreground">Doanh thu hom nay</p>
		...
		<div class="rounded-lg border border-border bg-card p-6 shadow-xs">
			<p class="text-sm text-muted-foreground">Dang cho</p>
	...
	{#if queue && (queue.waiting.length > 0 || queue.in_progress.length > 0)}
		<div class="mt-6 rounded-lg border border-border bg-card p-6">
			<h2 class="text-lg font-medium">Hang doi hien tai</h2>
			<div class="mt-4 space-y-2">
				{#each queue.in_progress as entry}
					<div class="flex items-center justify-between rounded-md bg-primary/5 px-4 py-2">
						...
						<span class="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">Dang rua</span>
					</div>
				{/each}
				{#each queue.waiting as entry}
					<div class="flex items-center justify-between rounded-md px-4 py-2">
						...
						<span class="text-xs text-muted-foreground">~{entry.estimated_wait_minutes} phut</span>
					</div>
				{/each}
			</div>
		</div>
